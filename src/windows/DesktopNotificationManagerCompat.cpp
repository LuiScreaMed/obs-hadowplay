...
HRESULT RegisterAumidAndComServer(const wchar_t *aumid, GUID clsid)
{
	...
	RETURN_IF_FAILED(
		charWritten > 0 ? S_OK : HRESULT_FROM_WIN32(::GetLastError()));
...
HRESULT RegisterComServer(GUID clsid, const wchar_t exePath[])
{
	...
	return HRESULT_FROM_WIN32(::RegSetKeyValue(
		HKEY_CURRENT_USER, subKey.c_str(), nullptr, REG_SZ,
...
HRESULT CreateToastNotification(IXmlDocument *content,
...
bool IsRunningAsUwp()
{
	if (!s_hasCheckedIsRunningAsUwp) {
		...
		LONG result = GetPackageFamilyName(GetCurrentProcess(), &length,
						   packageFamilyName);
...
HRESULT DesktopNotificationHistoryCompat::GetHistory(
	ABI::Windows::Foundation::Collections::IVectorView<ToastNotification *>
		**toasts)
{
